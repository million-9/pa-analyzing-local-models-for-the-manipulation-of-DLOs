...
from win32cryptcon import CMSG_ENCODE_HASHED_SUBJECT_IDENTIFIER_FLAG
...
mse=0
ROPE_CONTROLS = np.zeros((2000,100,3))
ROPE_MSE = np.zeros(2000)
PROGRAM_STEP = 0
ROPE_POSITIONS_HISTORY = np.zeros((20000, 100, 3))
GRIPPER_POSITIONS = np.zeros((20000,2, 7))
STATE = 0
time = 0
iterations = 0
t= 0
...
def compute_gripper_control():

    global XC_GRIPPER,X_ROPE,optimized_G, XD_ROPE

    # Compute direction vector (dc) and normalize it
    dc = (XD_ROPE - X_ROPE)
    # Compute the required gripper movement using the normalized dc
    mdc = np.linalg.norm(dc)
    d_r = np.dot(dc.flatten(), optimized_G)
    magnitude = np.linalg.norm(d_r)

    #limiting the magnitutde within 1 to 5
    if magnitude <2:
        d_r = (d_r/magnitude)*2
    if magnitude > 5:
        d_r = (d_r / magnitude)* 5
    # Compute the new gripper pose
    g_i = XC_GRIPPER
    g_r = g_i + d_r
    if g_r[2] < 15:
        g_r[2] = 15+(15-g_r[2])
    if g_r[2] > 45:
        g_r[2] = 45-(g_r[2]-45)
    g_r[3] = np.radians(180 - g_r[3])
    return np.asarray(g_r)
...
def get_gripper_pose(gripper):
    return np.hstack((gripper.get_pose().copy()[:3], quaternion_to_euler_angles(gripper.get_pose().copy()[3:])[2]))
...
class RopeAccessController(Sofa.Core.Controller):
    ...
    def __init__(self,train_interval,max_data,*args, **kwargs):
        Sofa.Core.Controller.__init__(self,*args, **kwargs)
        self.rope = kwargs.get("rope") # Pass the gripper controller
        self.gripper1 = kwargs.get("gripper1")
        self.gripper2 =kwargs.get("gripper2")
    ...
    def onAnimateEndEvent(self, event):
        global  ROPE_POSITIONS_HISTORY,GRIPPER_POSITIONS,time,ROPE_CONTROLS,X_ROPE,t,optimized_G,ROPE_MSE,mse

        X_ROPE = self.rope.get_positions()
        t = t + 1

        if STATE <3:

            ROPE_POSITIONS_HISTORY[time] = self.rope.get_positions()
            GRIPPER_POSITIONS[time,0] = self.gripper1.get_pose()
            GRIPPER_POSITIONS[time, 1] = self.gripper2.get_pose()
            time += 1

            if self.iterations >= ROPE_CONTROLS.shape[0]:
            # Expand the array if there's no space
                ropepose = np.expand_dims(self.rope.get_positions(), axis=0)  # Shape (1, 100, 3)
                ROPE_CONTROLS= np.concatenate((ROPE_CONTROLS, ropepose), axis=0)
            else:
                ROPE_CONTROLS[self.iterations]= X_ROPE
        self.iterations += 1

        if STATE == 2:
            #800-2000 works!, just not enough rope length here
            #low mse works just not enough rope length
            if t >= self.train_interval:
                if time > self.max_data:
                    optimized_G, delta_c = model_train(ROPE_POSITIONS_HISTORY[time - self.max_data:time, :85, :],
                                                       GRIPPER_POSITIONS[time - self.max_data:time, :1, :])
                else:
                    optimized_G, delta_c = model_train(ROPE_POSITIONS_HISTORY[:time, :85, :],
                                                       GRIPPER_POSITIONS[:time, :1, :])
                check = np.linalg.norm(delta_c, axis=(1,2))
                t = 0

        if STATE == 4:
            np.save("controlpath.npy", ROPE_CONTROLS[:self.iterations])
            np.save("time_history.npy", step_history)
            np.save("controlmse.npy", mse_history)
...
class GripperAccessController(Sofa.Core.Controller):
    ...
    def __init__(self,waypoints,desired_postion,threshold, *args, **kwargs):
        Sofa.Core.Controller.__init__(self, *args, **kwargs)
        self.desired_position = desired_postion
        self.gripper1 = kwargs.get("gripper1")
    ...
    def onAnimateEndEvent(self, event):
        ...
        dt = 0.01
        global STATE, X_GRIPPER, X_GRIPPER_LAST, V_GRIPPER,optimized_G,time,t,step_history,X_ROPE,XD_ROPE,p,XC_GRIPPER,mse_history

        X_GRIPPER_LAST = X_GRIPPER
        X_GRIPPER = self.gripper1.get_pose()
        XC_GRIPPER = get_gripper_pose(self.gripper1)
        X_ROPE = self.rope.get_positions()[:85, :]
        ...
        if STATE == 1:
            if plan_and_execute(gripper=self.gripper1, path=self.waypoints):
                optimized_G, delta_c = model_train(ROPE_POSITIONS_HISTORY[:time, :85, :],
                                                   GRIPPER_POSITIONS[:time, :1, :])
                p=time
                time=0
                STATE += 1
                print(mean_squared_error(X_ROPE,self.desired_position[0]))
                mse_history.append(mean_squared_error(X_ROPE,self.desired_position[0]))

        if STATE == 2:
            if self.index < len(self.desired_position):
                # Set the current desired position
                XD_ROPE = self.desired_position[self.index]
                if self.index > (len(self.desired_position)-3) :
                    self.threshold = 0.5
                if plan_and_execute(gripper=self.gripper1, path=np.asarray([compute_gripper_control()])):
                    mse = mean_squared_error(X_ROPE, XD_ROPE)
                    mse_history.append(mse)
                    print("Name: " ,mse , ", index: " ,self.index)
                    if mse < self.threshold:
                        step_history.append(p + time)
                        self.index = self.index + 1
                        mse_history.append(mean_squared_error(X_ROPE, XD_ROPE))
...
def createScene(
    ...
    add_args: list = [],
    desired_position: np.array = np.zeros(2),
    threshold: float = None , # Assigning a float value
    max_data: int= 0,
    ...
    poses_for_rope = poses_for_linear_rope(length=210, num_points=100, start_position=np.array([0.0, 70.0, 19.0]))
    ...
    board_limits = {
        "low": np.array([5., 5., -5.]),
        "high": np.array([55.+40.+40.+40, 65.+40.+40.+40, 0.]),
    }
    grid_shape = (10, 10, 2)
    ...
    left_gripper = ArticulatedGripper(
        ...
        rcm_pose=np.array([4.0, 70.0, 85.0, 0.0, 180.0, 0.0]),
    ...
    right_gripper = ArticulatedGripper(
        ...
        # Initialize gripper state [gripper x, gripper y, gripper z, unused angle, yaw, unused angle]
        rcm_pose=np.array([178.5, 70., 85., 0.0, 180., 0.0]),
        collision_spheres_config={
    ...
    root_node.addObject(RopeAccessController(train_interval,max_data,'RopeAccessor', name='ropeAccessor', rope=rope, gripper1=left_gripper, gripper2=right_gripper ))
    root_node.addObject(GripperAccessController(waypoints,desired_position,threshold,'GripperAccessorLeft', name='gripperAccessorLeft', gripper1=left_gripper ,rope=rope,gripper2=right_gripper))

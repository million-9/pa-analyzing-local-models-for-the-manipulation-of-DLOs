...
class RopeAccessController(Sofa.Core.Controller):
    ...
    def onAnimateEndEvent(self, event):
        global  ROPE_POSITIONS_HISTORY,GRIPPER_POSITIONS,time,ROPE_CONTROLS,X_ROPE,t,optimized_G
        ...
        if STATE == 2:
            ...
            if t == self.train_interval:
                if time > self.max_data:
                    optimized_G, delta_c = model_train(ROPE_POSITIONS_HISTORY[time - self.max_data:time, :48, :],
                                                       GRIPPER_POSITIONS[time - self.max_data:time, :1, :])
                else:
                    optimized_G, delta_c = model_train(ROPE_POSITIONS_HISTORY[:time, :48, :],
...
class GripperAccessController(Sofa.Core.Controller):
    ...
    def onAnimateEndEvent(self, event):
        ...
        X_ROPE = self.rope.get_positions()[:48, :]
            ...
            if self.index < len(self.desired_position):
                ...
                if plan_and_execute(gripper=self.gripper1, path=np.asarray([compute_gripper_control()])):
                    ...
                    if mse < self.threshold:
                        ...
                        print(mean_squared_error(X_ROPE, XD_ROPE))
